...
export const IntelTab = ({ user, initialData }: { user: any, initialData?: Tab[] }) => {
  ...
  const fetchTabs = async () => {
    const res = await fetch('/api/intel-tabs');
  ...
  const startEdit = async (tab: Tab) => {
    ...
    await fetch(`/api/intel-tabs/${tab.id}`, {
      method: 'PUT',
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      ...
      for (let i = 0; i < newTabs.length; i++) {
        await fetch(`/api/intel-tabs/${newTabs[i].id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const removeTab = async () => {
    ...
    await fetch(`/api/intel-tabs/${contextMenu.tabId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <DebugLabel label="Intel Module" className="flex flex-col flex-grow border-t border-accent/20 overflow-hidden relative">
